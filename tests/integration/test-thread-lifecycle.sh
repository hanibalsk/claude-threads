...
test_thread_create_modes() {
    ...
    (( ++TESTS_RUN )) || true
    if echo "$output" | grep -qE "thread-[a-f0-9]+"; then
...
test_thread_status_transitions() {
    ...
    assert_equals "$status" "pending" "Initial status is pending"
...
test_thread_list_filters() {
    ...
    .claude-threads/bin/ct thread create filter-pending --mode automatic >/dev/null 2>&1 || true
    .claude-threads/bin/ct thread create filter-running --mode automatic >/dev/null 2>&1 || true
    .claude-threads/bin/ct thread create filter-completed --mode automatic >/dev/null 2>&1 || true
    .claude-threads/bin/ct thread create filter-failed --mode automatic >/dev/null 2>&1 || true

    # Update statuses
    sqlite3 .claude-threads/threads.db "UPDATE threads SET status = 'running' WHERE name = 'filter-running'" 2>/dev/null || true
    sqlite3 .claude-threads/threads.db "UPDATE threads SET status = 'completed' WHERE name = 'filter-completed'" 2>/dev/null || true
    sqlite3 .claude-threads/threads.db "UPDATE threads SET status = 'failed' WHERE name = 'filter-failed'" 2>/dev/null || true

    # Test listing by status
    log_test "Filtering threads by status..."
    assert_output_contains ".claude-threads/bin/ct thread list pending" "filter-pending" "List pending works"
    assert_output_contains ".claude-threads/bin/ct thread list running" "filter-running" "List running works"
    assert_output_contains ".claude-threads/bin/ct thread list completed" "filter-completed" "List completed works"
    assert_output_contains ".claude-threads/bin/ct thread list failed" "filter-failed" "List failed works"

    # Test --all flag
    log_test "Testing --all flag..."
    local output
    output=$(.claude-threads/bin/ct thread list --all 2>&1) || true

    (( ++TESTS_RUN )) || true
    if echo "$output" | grep -q "filter-pending" && echo "$output" | grep -q "filter-completed"; then
        log_pass "List --all shows all statuses"
    else
        log_fail "List --all missing threads: $output"
    fi
...
test_thread_events_integration() {
    ...
    # Publish THREAD_STARTED event
    log_test "Publishing THREAD_STARTED from thread..."
    .claude-threads/bin/ct event publish THREAD_STARTED "{\"thread_id\": \"$thread_id\"}" --source "$thread_id" >/dev/null 2>&1 || true

    # Check event exists
    local event_exists
    event_exists=$(sqlite3 .claude-threads/threads.db "SELECT COUNT(*) FROM events WHERE source_thread_id = '$thread_id' AND type = 'THREAD_STARTED'" 2>/dev/null) || event_exists=0
    assert_equals "$event_exists" "1" "THREAD_STARTED event recorded"

    # Publish THREAD_COMPLETED event
    log_test "Publishing THREAD_COMPLETED from thread..."
    .claude-threads/bin/ct event publish THREAD_COMPLETED "{\"thread_id\": \"$thread_id\", \"result\": \"success\"}" --source "$thread_id" >/dev/null 2>&1 || true

    event_exists=$(sqlite3 .claude-threads/threads.db "SELECT COUNT(*) FROM events WHERE source_thread_id = '$thread_id' AND type = 'THREAD_COMPLETED'" 2>/dev/null) || event_exists=0
    assert_equals "$event_exists" "1" "THREAD_COMPLETED event recorded"

    # List events from thread
    log_test "Listing events from thread..."
    assert_output_contains ".claude-threads/bin/ct event list --source $thread_id" "THREAD_STARTED" "Thread events are listable"
...
test_thread_parent_child() {
    ...
    local parent_id
    parent_id=$(echo "$output" | grep -oE "thread-[a-f0-9]+" | head -1) || parent_id=""
    ...
    log_test "Creating child thread with parent..."
    output=$(.claude-threads/bin/ct thread create child-thread --mode automatic --parent "$parent_id" 2>&1) || true
    local child_id
    child_id=$(echo "$output" | grep -oE "thread-[a-f0-9]+" | head -1) || child_id=""
    ...
    assert_equals "$stored_parent" "$parent_id" "Parent reference stored correctly"
...
test_thread_error_handling() {
    ...
    sqlite3 .claude-threads/threads.db "UPDATE threads SET status = 'failed', error_message = 'Test error' WHERE id = '$thread_id'" 2>/dev/null || true

    # Check error is stored
    local error_msg
    error_msg=$(sqlite3 .claude-threads/threads.db "SELECT error_message FROM threads WHERE id = '$thread_id'" 2>/dev/null) || error_msg=""
